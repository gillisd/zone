...
module Zone
  module Pattern
    ...
    def process(input : Input, output : Output, transformation : Proc(String, String?), logger : Log)
      input.each_line do |line|
        process_line(line, input.from_arguments?, output, transformation, logger)
      end
    end

    private def process_line(line : String, from_arguments : Bool, output : Output, transformation : Proc(String, String?), logger : Log)
      if line.empty?
        ...
        elsif result == line && !from_arguments
          output.puts(line)
        else
          output.puts(result)
        end
    ...
    private def replace_timestamps(line : String, output : Output, transformation : Proc(String, String?), logger : Log) : String
      TimestampPatterns.replace_all(line, logger: logger) do |match, _pattern|
        transform_timestamp(match, output, transformation, logger)
      end
    end

    private def transform_timestamp(match : String, output : Output, transformation : Proc(String, String?), logger : Log) : String
