...
# Helper method for running zone CLI
...
describe "Git Log Integration" do
  ...
  it "parses git log default format" do
    input = <<-GIT
    ...
    output, status = run_zone_with_input(zone_bin, input, "--zone", "Tokyo")
  ...
  it "parses git log with negative offset" do
    input = "Date:   Fri Nov 14 14:54:35 2025 -0500"

    output, status = run_zone_with_input(zone_bin, input, "--zone", "UTC")
  ...
  it "parses git log with single digit day" do
    input = "Date:   Wed Nov 5 11:24:19 2025 -0500"

    output, status = run_zone_with_input(zone_bin, input, "--zone", "EST")
  ...
  it "parses git log fuller format" do
    input = <<-GIT
    ...
    output, status = run_zone_with_input(zone_bin, input, "--zone", "PST")
  ...
  it "parses git log with multiple commits" do
    input = <<-GIT
